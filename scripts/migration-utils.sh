...
case "$1" in
  create-led-brands)
    ...
    java ${JAVA_OPTIONS} -jar ${UTILS_JAR} --action=create-led-brands --migration.configuration=${CONFIG_FILE}
    ;;
  generate-merged-product-ids)
    printInfo "Generate merged product ids mapping (both in source system) ..."
    java ${JAVA_OPTIONS} -jar ${UTILS_JAR} --action=products-mapping --migration.configuration=${CONFIG_FILE}
    ;;
  migrate-products)
    printInfo "Migrate product ..."
    java ${JAVA_OPTIONS} -jar ${UTILS_JAR} --action=products-migrate --migration.configuration=${CONFIG_FILE}
    ;;
  upload-images)
    printInfo "Upload images to destination system ..."
    java ${JAVA_OPTIONS} -jar ${UTILS_JAR} --action=products-image-migrate --migration.configuration=${CONFIG_FILE}
    ;;
  migrate-product-tier-prices)
    printInfo "Migrate main product tier prices ..."
    java ${JAVA_OPTIONS} -jar ${UTILS_JAR} --action=products-tier-prices-migrate --migration.configuration=${CONFIG_FILE}
    ;;
  *)
    echo ""
