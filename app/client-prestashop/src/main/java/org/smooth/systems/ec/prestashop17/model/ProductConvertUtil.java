...
//    product.getProductImageUrls();
	...
	private LanguageContainer retrieveNames(PrestashopLanguageTranslatorCache cache, List<ProductTranslateableAttributes> attributes) {
		LanguageContainer container = new LanguageContainer();
		attributes.forEach(attr -> {
			container.getLanguages().add(new LanguageAttribute(cache.getLangId(attr.getLangCode()), attr.getName()));
		});
		return container;
	}

	private LanguageContainer retrieveDescriptions(PrestashopLanguageTranslatorCache cache, List<ProductTranslateableAttributes> attributes) {
		LanguageContainer container = new LanguageContainer();
		attributes.forEach(attr -> {
			container.getLanguages().add(new LanguageAttribute(cache.getLangId(attr.getLangCode()), attr.getDescription()));
		});
		return container;
	}

	private LanguageContainer retrieveShortDescriptions(PrestashopLanguageTranslatorCache cache, List<ProductTranslateableAttributes> attributes) {
		LanguageContainer container = new LanguageContainer();
		attributes.forEach(attr -> {
			container.getLanguages().add(new LanguageAttribute(cache.getLangId(attr.getLangCode()), attr.getShortDescription()));
		});
		return container;
	}

	private LanguageContainer retrieveLinksRewrite(PrestashopLanguageTranslatorCache cache, List<ProductTranslateableAttributes> attributes) {
		LanguageContainer container = new LanguageContainer();
		attributes.forEach(attr -> {
			container.getLanguages().add(new LanguageAttribute(cache.getLangId(attr.getLangCode()), attr.getFriendlyUrl()));
		});
		return container;
	}
